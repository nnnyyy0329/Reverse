...
private:
	void UpdateTransformTime();									// 変身タイマー更新関数
	void UpdateTransformCancelTime();							// 変身解除タイマー更新関数
	void PlayTransConnectionAnim(const char* animName);			// 変身関連アニメーション再生
	void CheckAbilityScreenActive(bool& isAbilityScreenActive);	// 能力選択画面がアクティブかチェック
	void ProcessAutoSwitch(bool isAbilityScreenActive);			// 変身中でなければ自動切り替えなどの処理
	void ProcessActivePlayer(bool isAbilityScreenActive);		// アクティブプレイヤーの処理
