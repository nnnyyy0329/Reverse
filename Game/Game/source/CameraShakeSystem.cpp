...
{
	// [-1,1] のランダム値
	float randX = mymath::RandomRange(-1.0f, 1.0f);
	float randY = mymath::RandomRange(-1.0f, 1.0f);

	// 時間経過で振動の大きさを減衰させるための係数
	float decay = 1.0f - (_currentTime / _stcShakeConfig.duration);	// 持続時間に対する現在の時間の割合を計算

	// 減衰後のオフセット
	float offsetX = randX * _stcShakeConfig.magnitude * decay;
	float offsetY = randY * _stcShakeConfig.magnitude * decay;

	if (_bUseHorizontalOnly)
	{
		offsetY = 0.0f;
	}

	// 計算した振動オフセットを返す
	return VGet(offsetX, offsetY, 0.0f);
}
...
void CameraShakeSystem::Apply(CameraBase* camera)
{
	...
	camera->ApplyShake(_shakeOffset);
}
