...
namespace 
{
	...
	constexpr auto SHOT_EXECUTE_TIME = 94.0f;			// 射撃実行時間
	constexpr auto SHOT_RECOVERY_TIME = 120.0f;			// 射撃後隙時間
	constexpr auto SHOT_INTERVAL_TIME = 90.0f;			// 射撃間隔時間
	constexpr float SHOT_BULLET_TIME = 67.0f;			// 実際に弾が発射される時間(アニメーション準拠)
	...
	constexpr auto ANIM_SPEED_FAST = 1.5f;
...
namespace Ranged
{
	...
	void ShotExecute::Enter(Enemy* owner)
	{
		...
		if (animManager)
		{
			animManager->ChangeAnimationByName("Senemy_attack_01", BLEND_FRAME, ANIM_PLAY_COUNT, ANIM_SPEED_FAST);
		}
	}
